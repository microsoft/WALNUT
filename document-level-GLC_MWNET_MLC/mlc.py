...
def sync_autograd_debug(loss, net, opt, args, retain_graph=False,
                        create_graph=False):  # DDP and AMP compatible autograd
...
def update_params_adam(params, grads, eta, opt, args, deltaonly=False, return_s=False):
    ...
    for i, param in enumerate(params):
        try:
            _step, _exp_avg, _exp_avg_sq = opt.state[param]['step'], opt.state[param]['exp_avg'], opt.state[param][
                'exp_avg_sq']
            if amsgrad:
...
def step_mwnet(main_net, main_opt, meta_net, meta_opt,
               ...
               eta, args):
    # META NET START
    # given current meta net, get instance weight
    logit_s = main_net(data_s)
    cost_s = F.cross_entropy(logit_s, target_s, reduction='none')
    v = meta_net(cost_s.detach().view(-1, 1))
    loss_s = (v * cost_s).mean()  # consistent with original code, papers mentions sum-norm?
    ...
    cost_s = F.cross_entropy(logit_s, target_s, reduction='none')
    v = meta_net(cost_s.detach().view(-1, 1)).detach()
    loss_s = (v * cost_s).mean()
