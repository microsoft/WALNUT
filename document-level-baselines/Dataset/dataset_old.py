    ...
    def __init__(self, hparams, train_status, is_only_clean=None, elmo_map=None):
        ...
        if train_status == "train":
            ...
            if is_agg_weak:
                ...
                agg_label = label_aggregation(data_interest['lf'][weak_index], hparams.agg_fn_str,
                ...
                weak_data['is_weak'] = [True] * len(weak_index)
        ...
        #                 hparams.clean_ratio) + "_{}".format(hparams.random_seed) + ".index"
        if train_status == "train":
            human_file_name = file_path.replace(".pt", "") + "_" + train_status + "_{}".format(hparams.clean_ratio) + "_{}".format(hparams.random_seed) + ".human"
        else:
            human_file_name = read_file + ".human"

        dump_data(self.data_interest, human_file_name)
    def __len__(self):
    ...
    def __init__(self, hparams, train_status, is_only_clean=None, elmo_map=None):
        ...
        if train_status == "train":
            ...
            if is_agg_weak:
                ...
                weak_data['label'] = agg_label
                weak_data['is_weak'] = [True] * len(agg_label)
        ...
        if train_status == "train":
            human_file_name = os.path.join(file_path, file_path.split("/")[-1] + "_" + train_status + "_{}".format(
                hparams.clean_ratio) + "_{}".format(hparams.random_seed) + ".human.json")
            # del self.data_interest['input_ids']
            # del self.data_interest['attention_m   ask']
        else:
            human_file_name = file_path + "/" + train_status + ".human.json"
        dump_data(self.data_interest, human_file_name)
    ...
    # for task in ['gossipcop']:
        ...
        # for random_seed in [123]:
            ...
            # for clean_ratio in [5, 50]:
                ...
                if task == "gossipcop":
                    dataset = FakeNewsDataset(hparams=args, train_status="train")
                else:
