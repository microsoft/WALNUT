...
def tflog2pandas(path: str, hyper:str) -> pd.DataFrame:
    ...
    if os.path.exists(model_config):
        ...
        try:
            for h in hyper_list:
                special_tag += "{}-{}-".format(h, model_config[h])
        except:
            th = 1
...
def many_logs2pandas(event_paths, compare_metric_list=("acc", val_type), hyper=""):
    ...
    all_logs['seed'] = all_logs['model_name'].apply(lambda x: int(x.split("-")[1]))
    if "clean_ratio" not in all_logs.loc[0, "model_name"]:
        all_logs['clean_ratio'] = -1
    else:
        try:
            all_logs['clean_ratio'] = all_logs['model_name'].apply(lambda x: int(float(x.split("-")[-2])))
        except:
            all_logs['clean_ratio'] = -100
    all_logs = all_logs[all_logs['metric'] == "Test/acc"]
    print(all_logs['model_name'].tolist())
    all_logs = all_logs.groupby("clean_ratio").agg({"value":["mean", "std", "count"]}).reset_index()
...
if __name__ == "__main__":
    args = ArgumentParser()
    args.add_argument("--exp_name", default="bert-large-uncased_clean+ab_20baseline_gossipcop", type=str)
    args.add_argument("--log_dir", default="/home/kshu/PycharmProjects/WittySnail/document-level/meta_logs", type=str)
    args.add_argument("--hyper", default="random_seed,clean_ratio", type=str)
